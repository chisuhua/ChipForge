...
> **Purpose**: Contract for the Phase 1.5 Wave 2 SoC demo — a CPU+MMU+Memory runner that executes ≥5 riscv-tests ELFs to `tohost=1`, validating TLB-refill mechanics end-to-end.
>
> **Scope note (revised)**: The demo runner SHALL construct the topology **directly in C++** (per the `test_rv32ui_runner.cpp:100-116` pattern), NOT via a JSON→Plugin instantiator. There is no `cf::soc::Topology::from_json` API — it does not exist in the codebase, and building a JSON→C++-class instantiator is out of scope for Wave 2 (≈3-5 days, deferred). The `soc/cpu_l1_mmu_demo.json` file is a **structural topology declaration only** (validated by a `tests/soc/` structural test), mirroring the existing `soc/mmu_minimal.json` "structural validation only, full simulation deferred" precedent. The L1 cache is **declared but NOT instantiated** in the runner — its instantiation is deferred to Wave 3 `cache-dse-sweep`.
