# cpu-plugin-template-thread-awareness Specification

## Purpose
Templates `RegFilePlugin`, `HazardPlugin`, and `BranchPredictorPlugin` on `N_REGS` and `N_THREADS` (with `static_assert` constraints `[1,128]` and `[1,4]` respectively), enabling Phase 5+ superscalar/SMT per-thread storage layouts. Default parameter values (`N_REGS=32, N_THREADS=1`) preserve the current ABI, ensuring existing callers compile unchanged.
## Requirements
