# ip-status-markdown-contract Specification

## Purpose
Requires the 5 zero-code IPs (`memory`, `interconnect`, `peripheral`, `tilecore`, `tilecopy`) to each have a `STATUS.md` marker (PLANNED or INITIAL DESIGN variant) at IP root, with a standardized template at `docs/templates/IP_STATUS_TEMPLATE.md`. Aligns zero-code IPs with roadmap implementation phases and prevents accidental scaffolding.
## Requirements
