# bridge-tick-pattern-rationale Specification

## Purpose
Distinguishes two component patterns: business Plugin-style (strict `cf::plugin::PluginBase`, no `tick()`) versus Bridge Adapter-style (relaxed, may implement `tick()` only for CppTLM `EventQueue` cycle callbacks with protocol-only body delegating to inner Plugin's `pb.run()`). Prevents abuse of `tick()` that would break D4 framework invariant.
## Requirements
