...
### 1.4 与 `cpptlm::CacheTLM` 对比（1-2 天）— ~~已废弃（2026-06-13）~~

> ⚠️ **此节已由 Phase 1.4 方法学复盘取代。** `cpptlm::CacheTLM` 是 `std::map` 全关联 stub，无法配置为 256×64B direct-mapped 对比基线（见 `roadmap-status.md` 行 23 范围修正）。详见 [`docs/methodology/plugin-style-design-methodology-v1.md`](../../../docs/methodology/plugin-style-design-methodology-v1.md)（352 行，6 维度 × 3 边界 + 6 B2 模式 + 3 B3 局限 + 5 Phase 6 任务）。

- [x] ~~创建 `soc/l1_cache_baseline.json`~~ — 废弃：CacheTLM stub 不可做 direct-mapped 对比
- [x] ~~用相同输入 trace 驱动两个配置~~ — 废弃
- [x] ~~对比关键指标~~ — 废弃
- [x] ~~Golden reference 测试~~ — 废弃：方法学复盘取代了功能等价基线
...
- [x] L1CachePlugin 在 TLM 模式下端到端跑通（`soc/l1_cache_minimal.json` 可执行）
- [x] ~~与 `cpptlm::CacheTLM` 同输入比对~~ — 废弃：Phase 1.4 方法学复盘取代，CacheTLM 是 std::map stub 不可对比（见 `roadmap-status.md` 行 23）
