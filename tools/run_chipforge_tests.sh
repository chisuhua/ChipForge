...
CHIPFORGE_TEST_REGEX='(test_(payload|pipe_node|pipe_builder|ctrl_link|hello_plugin|coexistence|plugin_lifecycle|mem_bundles))|(verify_plugin_decision)'

if [ -n "$INCLUDE_ALL" ]; then
    echo "[run_chipforge_tests] Running ALL 148 tests (139 CppHDL internal expected 'Not Run')..."
    ctest $VERBOSE
else
    echo "[run_chipforge_tests] Running ChipForge tests only (9 tests, expected 100% pass)..."
