...
class PicolibcHostMemory {
 public:
  ...
  // 字节写
  void write_byte(std::uint64_t addr, std::uint8_t val) {
    if (addr < kMemorySize) {
      mem_[addr] = val;
      check_tohost(addr, val);
    }
  }

  // 字节读
  std::uint8_t read_byte(std::uint64_t addr) const {
    if (addr < kMemorySize) return mem_[addr];
    return 0;
  }

  // 字写 (32-bit, little-endian)
  void write_word(std::uint64_t addr, std::uint32_t val) {
    if (addr + 3 < kMemorySize) {
  ...
  // 字读
  std::uint32_t read_word(std::uint64_t addr) const {
    if (addr + 3 < kMemorySize) {
      return (std::uint32_t)mem_[addr + 0]
           | ((std::uint32_t)mem_[addr + 1] << 8)
           | ((std::uint32_t)mem_[addr + 2] << 16)
           | ((std::uint32_t)mem_[addr + 3] << 24);
    }
    return 0;
  }
 ...
 private:
  ...
  // 检测 tohost 写入
  void check_tohost(std::uint64_t addr, std::uint8_t val) {
    if (addr == kTohostAddr && val != 0) {
      tohost_ = val;
